function [] = eddyBioVis(srcData, property, dataFilePath, allEddy, eddyHistory,eddyPathIndex)
...
for eddyTimeIndex = 1:1:2
% for eddyTimeIndex = 1:1:size(currEddyPath,1)
    eddyTimeStepTemp = currEddyPath(eddyTimeIndex,15);
    eddyIndexTemp = currEddyPath(eddyTimeIndex,16);
%     eddyData = load(dataFilePath+"Seperated Structures/Frame_"+num2str(eddyTimeStepTemp)+"_eddy_"+num2str(eddyIndexTemp)+"_statistic.uocd");
    ...
    rotationFlag = currEddyPath(eddyTimeIndex,14);
    
    
    eddyData = readEddyData(dataFilePath, rotationFlag, eddyTimeStepTemp, eddyIndexTemp);

    depthRecord(eddyTimeIndex,1) = max(eddyData(:,5));
    depthRecord(eddyTimeIndex,4) = eddyTimeStepTemp;
    [~,depthRecord(eddyTimeIndex,2)] = ismember(depthRecord(eddyTimeIndex,1),z_val);
end

depthRecord(1:end-1,3) = abs(depthRecord(1:end-1,2) - depthRecord(2:end,2))./(min(depthRecord(1:end-1,2), depthRecord(2:end,2)));
suddenChangeTimesteps = depthRecord(depthRecord(:,3)>=suddenChangeThresh, :);

for caseIndex = 1:1:2
% for caseIndex = 1:1:size(suddenChangeTimesteps,1)
    close all;
    currentSuddenChangeCase = suddenChangeTimesteps(caseIndex,:);
    prevTimestep = currentSuddenChangeCase(4);
    nextTimestep = currentSuddenChangeCase(4)+1;
    prevEddy = currEddyPath(currEddyPath(:,15)==prevTimestep,:);
    nextEddy = currEddyPath(currEddyPath(:,15)==nextTimestep,:);
    
    [~,prevEddy_centerX] = min(abs(prevEddy(1)-x_val));
    [~,nextEddy_centerX] = min(abs(nextEddy(1)-x_val));

    prevEddyIndex = prevEddy(16);
    nextEddyIndex = nextEddy(16);
    
    prevObjIndex = currEddyPath(prevTimestep,16);

    
    rotationFlag = currEddyPath(prevTimestep,14);
...
%     cutoffDepth = min(prevEddy_depth,nextEddy_depth);
%     if(prevEddy_depth < nextEddy_depth)
%         cutoffDepthFlag=-1;
%     elseif(prevEddy_depth > nextEddy_depth)
%         cutoffDepthFlag = 1;
%     else
%         cutoffDepthFlag = 0;
%     end
...
%     ax_isosurface_next = subplot(1,2,2);
    ...
    u_val_prev = ncread(srcData, property.u, [1,1,1,prevTimestep],[length(x_val),length(y_val),length(z_val),1]);
    v_val_prev = ncread(srcData, property.v, [1,1,1,prevTimestep],[length(x_val),length(y_val),length(z_val),1]);
    temp_val_prev = ncread(srcData, property.temp, [1,1,1,prevTimestep],[length(x_val),length(y_val),length(z_val),1]);
    ...
    vals = [temp_inside_mean, temp_middle_mean, temp_outer_mean];  % 每个环的值
...
%     set(ringFig, 'AlphaData', ~isnan(temp_result));
    ...
    color = cmap(1,:);
    alpha = 0.5;
    owIndividualIsosurface(ax_isosurface_prev, gridX, gridY, gridZ, minX_index_prev, maxX_index_prev, minY_index_prev, maxY_index_prev, temp_val_prev, voxel_threshold,value_threshold, color,alpha);
...
%     owIndividualIsosurface(ax, gridX, gridY, gridZ, minX_index, maxX_index, minY_index, maxY_index, ow_val_prev, voxel_threshold,value_threshold, color,alpha);
    ...
    hold on
    plot3(ax_isosurface_prev, prevEddyCenter(:,1),prevEddyCenter(:,2),prevEddyCenter(:,3),'color',[1,0,0,0.5],'LineWidth',3,"DisplayName","eddy centerline");
    daspect([1 1 100]);    
    hold off;
