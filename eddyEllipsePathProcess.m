function [eddyHistory] = eddyEllipsePathProcess(allEddy,eddyPath, ...
    G,livingFrameLimit)
...
%         xlabel('node linages')
        ...
        for pathIndex=1:1:length(eddyIndividualPaths)
            ...
            for thisEddyIndex=1:1:eddyTimeFrames
                % Eddy rotation check
                eddyCheck=allEddy((allEddy(:,15)==thisEddy(1,thisEddyIndex))&(allEddy(:,16)==thisEddy(2,thisEddyIndex)-1),:);
                
                eddyHistoryTemp = [eddyHistoryTemp;eddyCheck];
