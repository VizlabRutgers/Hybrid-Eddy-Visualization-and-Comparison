function [allEddy, clockEddy, conterclockEddy] = readEddyDetection(dataFilePath)
...
%     hold on
    ...
    clockEddy=[clockEddy;[data(1,:),max(data(:,5))]];
...
Path = dataFilePath+"counterclockwise/";
file = dir(fullfile(Path,'*.uocd'));
filenames = {file.name};
...
%     hold on
    ...
    conterclockEddy=[conterclockEddy;[data(1,:),max(data(:,5))]];
    allEddy=[allEddy;[data(1,:),max(data(:,5))]];
end
end
